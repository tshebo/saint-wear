  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="relative bg-gradient-to-br from-yellow-400 via-yellow-500 to-yellow-400 py-20 lg:py-32 overflow-hidden">
        ...
        <div className="absolute right-0 top-0 w-64 h-64 bg-blue-600/10 rounded-full blur-3xl"/>
            ...
            <Card className="relative p-8 shadow-xl hover:shadow-2xl transition-all duration-500 bg-white/90 backdrop-blur-sm border-t-4 border-yellow-400 rounded-lg overflow-hidden group">
              ...
              {isSubmitted ? (
                <div className="text-center py-12 animate-fade-in">
                  ...
                  <h3 className={`${bebasNeue.className} text-3xl font-bold text-green-600 mb-4`}>
                    Message Sent!
                  </h3>
                  ...
                  <Button 
                    ...
                    className="bg-yellow-400 text-black hover:bg-blue-600 hover:text-white transition-all duration-300 group"
                  >
                ...
                <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                  <div className="space-y-4">
                      ...
                      <div key={field.id} className="relative group">
                        <label 
                          ...
                          className={`block text-sm font-medium transition-colors duration-300 ${
                            activeField === field.id ? 'text-yellow-500' : 'text-gray-700 group-hover:text-yellow-500'
                          }`}
                        >
                          {field.label}
                        </label>
                    ...
                    <div className="relative group">
                      <label 
                        htmlFor="message"
                        className={`block text-sm font-medium transition-colors duration-300 ${
                          activeField === 'message' ? 'text-yellow-500' : 'text-gray-700 group-hover:text-yellow-500'
                        }`}
                      >
                        Your Message
                      </label>
                  ...
                  <Button 
                    type="submit"
                    className="w-full bg-yellow-400 text-black hover:bg-blue-600 hover:text-white transition-all duration-300 transform hover:scale-102 group"
                  >
                    Send Message
            ...
            <div className="space-y-8">
              <Card className="overflow-hidden hover:shadow-xl transition-all duration-500 bg-white/90 backdrop-blur-sm border-t-4 border-yellow-400 group">
                <div className="p-8 space-y-6 relative">
                  <div className="absolute inset-0 bg-gradient-to-br from-yellow-400/5 to-blue-600/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500"/>
                  {contactInfo.map(({ icon: Icon, text, href, label }, index) => (
                    <a 
                      key={index}
                      href={href}
              ...
              <Card className="overflow-hidden hover:shadow-xl transition-all duration-500 bg-white/90 backdrop-blur-sm border-t-4 border-yellow-400 group">
                <CardContent className="p-8 relative">
                  <div className="absolute inset-0 bg-gradient-to-br from-yellow-400/5 to-blue-600/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500"/>
                  <h3 className={`${bebasNeue.className} text-2xl font-bold text-blue-600 mb-6 relative z-10`}>
                    Connect With Us
                  </h3>
